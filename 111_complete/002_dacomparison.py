...
User choices: emittance blowup at the end of the line, radiation ON/OFF tapering ON/OFF ECS settings (fixed) and injection type
betatron/synchrotron or hybrid 
Output: survival fraction within DA of HER for fixed ECS lag, voltage
=============================================
...
######################################### DA Filepaths ##########################################
#200_8:  Uncomment below for the 200_8 lattice DA
...
#100_3: Uncomment below for the 100_3 lattice DA
...
#################################### Lattice Injection Optics Parameters from 4D twiss #######################
...
# initial_distribution    = xt.Table({
#     "name":     np.arange(initial_distribution[:, 0].shape[0]),
#     "x":        +1 * initial_distribution[:, 0] - np.mean(initial_distribution[:, 0]),
#     "px":       +1 * initial_distribution[:, 1] - np.mean(initial_distribution[:, 1]),
#     "y":        +1 * initial_distribution[:, 2] - np.mean(initial_distribution[:, 2]),
#     "py":       +1 * initial_distribution[:, 3] - np.mean(initial_distribution[:, 3]),
...
print(f"Mean x post ref transform:  ", np.mean(tracked_beam.x[tracked_beam.state == 1]))
print(f"Mean px post ref transform: ", np.mean(tracked_beam.px[tracked_beam.state == 1]))
...
axs[0].set_xlabel(r"$A_{x}$", fontsize=15)
axs[0].set_ylabel(r"$A_{y}$",fontsize=15)

axs[1].set_xlabel(r"$A_{z}$",fontsize=15)
axs[1].set_ylabel(r"$A_{x}$",fontsize=15)

axs[2].set_xlabel(r"$A_{z}$",fontsize=15)
axs[2].set_ylabel(r"$A_{y}$",fontsize=15)

axs[0].set_title("XY", fontsize=15)
axs[1].set_title("ZX", fontsize=15)
axs[2].set_title("ZY", fontsize=15)

fig.suptitle(f"Surviving Fraction: {np.sum(within) / within.size * 100:.4f}% for ECS Lag = {line['acw_lag']}° and ECS Voltage = {line['volt_acw']/1e6} MV \n Radiation is {Radiation} and Tapering is {Tapering}", fontsize=15)
