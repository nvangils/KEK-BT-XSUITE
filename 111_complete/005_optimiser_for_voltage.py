...
Output: 
MAXIMAL survival fraction within DA of HER for fixed ECS lag scanning the ECS voltage
Optimiser steps show dependence of survivability on ECS voltage for a fixed ECS lag 
=============================================
...
EMITTANCE_Y_MEASURED = 300e-6/14000	 # MEASURED EMITTANCE AT BTE2 IN NORMALISED
#Line configuration:
...
set_voltage_bounds = (18E6, 20E6)    # BOUNDS FOR THE OPTIMISER
...
######################################### Prepare voltage Scan ###########################################
survival_results = []
######################################### voltage Optimization ###########################################


# To record optimizer path
eval_history = {"volt": [], "surv": []}

def survival_fraction_for_voltage(volt_val):
    """Compute the surviving fraction for a given voltage value."""
    line["volt_acw "] = volt_val
    print(f"\nEvaluating voltage: {volt_val:.2f} V")
 # Reset tracked beam
    initial_distribution = np.loadtxt(particles_filepath)
    ...
    survival_results.append((volt_val, survival_fraction))
    eval_history["volt"].append(volt_val)
    eval_history["surv"].append(survival_fraction)

    print(f"Voltage {volt_val:.2f} → Survival: {survival_fraction*100:.3f}%")
    #survival_results.append((voltage_val, survival_fraction))
    return float(-survival_fraction)

# Run the optimizer
res = minimize_scalar(survival_fraction_for_voltage, bounds=set_voltage_bounds, method='bounded', options={'xatol': 0.01})

opt_volt = res.x
opt_fraction = -res.fun
print("\n==================== OPTIMUM FOUND ====================")
print(f"Best voltage = {opt_volt:.4f}, Survival = {opt_fraction*100:.4f}, lag: {line["lag_acw"]}%")
print("=======================================================\n")

######################################### Find optimum ###########################################

# Extract voltage values and survival fractions
survival_results = np.array(survival_results)
volts = survival_results[:, 0]
fractions = survival_results[:, 1]

# Find optimum
opt_idx = np.argmax(fractions)
opt_volt = volts[opt_idx]
opt_fraction = fractions[opt_idx]

# --- Plot ---
plt.figure(figsize=(10, 6))

# Plot survival fraction vs voltage
plt.plot(volts, fractions*100, "o-", lw=2, color="blue", label="Surviving fraction")

# Highlight the optimum
plt.scatter(opt_volt/1E6, opt_fraction*100, color="red", s=100, zorder=10, label=f"Maximum: {opt_fraction*100:.2f}% at {opt_volt:.2f}°")
plt.axvline(opt_volt/1E6, color="red", linestyle="--", lw=2)

# Labels and grid
plt.xlabel("Volt [MV]", fontsize=12)
plt.ylabel("Surviving fraction [%]", fontsize=12)
plt.title("Injection Survivability vs Voltage", fontsize=14)
...
################## plot optimiser path ###########################################
# === Extract results ===
volts = np.array(eval_history["volt"]) / 1E6  # Convert to MV
surv = np.array(eval_history["surv"]) * 100

# Sort for smooth line
idx = np.argsort(volts)
volts_sorted = volts[idx]
surv_sorted = surv[idx]

opt_volt = volts[np.argmax(surv)]
...
ax.plot(volts_sorted, surv_sorted, "-", color="blue", lw=2, alpha=0.7, label="Survival vs Voltage")

# Points visited by optimizer (in order)
ax.scatter(volts, surv, c=np.arange(len(volts)), cmap="plasma", s=80, edgecolor="k", lw=0.5, label="Optimizer path")

# Highlight optimum
ax.scatter(opt_volt/1E6, opt_surv, color="red", s=100, zorder=10, label=f"Optimum: {opt_surv:.2f}% @ {opt_volt:.2f}°")
ax.axvline(opt_volt/1E6, color="red", linestyle="--", lw=1)

# Labels and aesthetics
ax.set_xlabel("Volt [MV]", fontsize=12)
ax.set_ylabel("Survival Fraction [%]", fontsize=12)
ax.set_title("Injection Survivability vs Voltage (with optimiser path)", fontsize=14)
...
print(f"\n=== Re-running DA comparison for optimal voltage {opt_volt} ===")
line["volt_acw "] = opt_volt
initial_distribution = np.loadtxt(particles_filepath)
initial_distribution = xt.Table({
    "name":     np.arange(initial_distribution[:, 0].shape[0]),
    "x":        initial_distribution[:, 0] - np.mean(initial_distribution[:, 0]),
    "px":       initial_distribution[:, 1] - np.mean(initial_distribution[:, 1]),
    "y":        initial_distribution[:, 2] - np.mean(initial_distribution[:, 2]),
    "py":       initial_distribution[:, 3] - np.mean(initial_distribution[:, 3]),
    "zeta":     initial_distribution[:, 4] - np.mean(initial_distribution[:, 4]),
    "delta":    initial_distribution[:, 5] - np.mean(initial_distribution[:, 5]),
    "state":    np.full(initial_distribution[:, 0].shape, 1)})
